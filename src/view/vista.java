...
public class vista extends JFrame {
	 ...
	 * que se usaran en la generacion del reporte las
	 * cuales almacenaran mensajes o 
	 * result de conexiones o consultas a la BBDD
	 */
	...
	/**
	 * se instancia la conexion SFTP que 
	 * realizara la tranferencia y carga de los
	 * archivos RTRA
	 */
	private ConexionFtp cone = new ConexionFtp();

	/**
	 * realiza la ejecucion del 
	 * java cargara la interfaz
	 *  con sus elementos
	 * 
	 */
	...
	public vista() {
		...
		f.addWindowListener(new WindowAdapter() {
			...
			public void windowOpened(WindowEvent e) {
				...
				/**
				 * Se valida la conexion a la BBDD 
				 * se puede generar errores por no estar
				 * coenectado a la VPN
				 */
				try {
				...
				/**
				 * valida las cargas en cuanto se ejecuta
				 *  el jar de no tener informacion
				 * realizara las validacion de los
				 *  fichero y despues su carga
				 */
				if (victoria.equals(ConstantsUtil.NOCARGA) || dolphing.equals(ConstantsUtil.NOCARGA)
					...
					try {
						/**
						 * valida la carga de los RTRA en 
						 * cuanto se ejecuta el jar y lo mostrara en el
						 * textfiel
						 */
		...
		f.setIconImage(Toolkit.getDefaultToolkit()
				.getImage("src/util/santecLogo.png"));
		f.setSize(357, 338);
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				 ...
				 * mediante la ejecucion del query y validacion
				 * de los intrumentos
				 * 
				 */
				...
				try {
					...
					if (!textField.getText().isEmpty()) {
						...
						if (res.equals("No existen registros para este grupo en la interfaz CONSULTA")) {
							...
							/**
							 * se genera el nombre de la interfaz final que
							 *  contendra la informacion ya con
							 * los estilos requeridos por el Usuario
							 */
							File directorio = new File(directoryName + "\\" + grupo + "-" + Empresa + "\\" + anio + "\\"
									+ mesnum + " ) " + last3 + "\\" + part1 + ".xls");
							/**
							 * Se valida si el directorio 
							 * existe para depositar el fichero 
							 * de no ser asi lo genera y de posita 
							 * en la ruta armada
							 */
							if (!directorio.exists()) {
								...
								textField1.update(textField1.getGraphics());
								/**
								 * se valida si el rtra ya existe y se
								 *  vuelva a dar clic en generar RTRA por
								 * algun error
								 */
					 ...
					 *  ocurrio un problema al generar la interfaz
					 */
				} catch (Exception e1) {
					esperar();
		...
		/**
		 * texfiel declarado con todas sus propiedades
		 * en cuestion a las dimenciones y visibilidad
		 */
		...
		/**
		 * lblNewLabel declarado con todas sus propiedades
		 * en cuestion a las dimenciones y visibilidad
		 */
		...
		/**
		 * boton Salir termina con el 
		 * proceso inmediatamente en 
		 * cuanto lo presionan
		 */
		...
		/**
		 * boton en el calendario que se 
		 * encargara de realizar un reporceso realizando
		 * todas las validaciones como 
		...
		dateChooser.getCalendarButton().addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				/**
				 * Evento de doble click que 
				 * validara cargas de los fichero
				 * en la BBDD en dado
				...
				if (e.getClickCount() == 2 && !e.isConsumed()) {
					...
					/**
					 * se valida que el usuario no haga 
					 * un proceso de un dia siguiente al actual de
					 * ser asi mostrara el mensaje en
					 *  pantalla que no es permitido
					 */
					if (fechaVal.compareTo(fechaAct) > 0) {
						...
						/**
						 * se instancia la conexion que va a 
						 * obtener la ultimas carga de los insumos
						 * Victoria y Dolphing
						 */
							 ...
							 * carga de no ser asi realiza la ejecucion de la
							 * clase SFTP que tranfiere y 
							 * ejecuta las cargas de los RTRA
							 */
							if (victoria.equals(ConstantsUtil.NOCARGA) || dolphing.equals(ConstantsUtil.NOCARGA)
									|| !date.trim().equals(victoria.trim()) || !date.trim().equals(dolphing.trim())) {
								String res = Validaficheros.validaFicherosDolphinVictoria(dateChooser.getDate().getTime());
								 ...
								 *  obtenida del data chose calendar para
								 * validar su carga en la BBDD y 
								 * lo muestra en el texfield
								 */
								textField1.setText(
		...
		/**
		 * dateChooser declarado con todas sus propiedades
		 * en cuestion a las dimenciones y visibilidad
		 */
		 ...
		 * grafica teexfiel progressbar Estilos
		 */
